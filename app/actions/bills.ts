...
/**
 * Builds everything the PDF needs to print barcode stickers for a bill.
 * Barcodes are rendered server-side (bwip-js) into data URIs so the PDF has no
 * runtime network/auth dependency — the previous approach embedded a relative
 * /api/.../barcodes URL that the client PDF renderer had to refetch.
 */
  ...
  try {
    ...
    return {
      bill: {
        ...
        tenant_name: (bill.tenant as { name: string } | null)?.name ?? 'Mahendra Saree House',
