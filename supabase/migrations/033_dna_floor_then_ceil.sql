-- 033_dna_floor_then_ceil.sql
---
-- DNA rounding: floor-then-ceil so a value whose integer part is already a
-- multiple of 5 stays at that multiple (decimals are dropped, not rounded up).
--
--   2070.34 → 2070 (integer part 2070 is 5×414, keep it)
--   2051    → 2055 (not at a multiple, round up)
--   2054    → 2055
--   2056    → 2060
--   2050    → 2050 (already at a multiple, stays)
--
-- Formula: ceil(floor(v_dna_price) / 5.0) * 5
...
-- Backfill: recompute DNA with floor-then-ceil for all existing bill_items.
DO $$
DECLARE
  r RECORD;
  v_ma numeric;
