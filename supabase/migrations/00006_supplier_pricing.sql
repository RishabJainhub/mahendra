-- Apply supplier pricing rules to bill line items on insert
...
BEGIN
  SELECT b.supplier_id, b.tenant_id
  INTO v_supplier_id, v_tenant_id
  FROM public.bills b
...
DROP TRIGGER IF EXISTS trg_compute_bill_item_pricing ON public.bill_items;

CREATE TRIGGER trg_compute_bill_item_pricing
  BEFORE INSERT ON public.bill_items
  FOR EACH ROW
  EXECUTE FUNCTION public.compute_bill_item_pricing();
